...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ToastView(
      // 2-A: wrap your app with ToastView
      textStyle: const TextStyle(fontSize: 19.0, color: Colors.white),
      backgroundColor: Colors.grey,
      radius: 10.0,
      child: MaterialApp(
        title: 'Demo for ToastView',
        theme: ThemeData(
      ...
      animationBuilder: const AnimRunBuilder(),
  ...
  Widget buildApp() {
    return MaterialApp(
      home: const MyHomePage(),
      builder: (_, Widget? child) => ToastView(child: child!),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Example for ToastView')),
      body: Center(
        child: Column(
